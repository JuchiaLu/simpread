...
export default class Auth extends React.Component {
    ...
    onChange( state, value, flag ) {
        let notify;
        const { dropbox, pocket, instapaper, linnk, evernote, onenote, gdrive, jianguo,wallabag, yuque, notion, youdao, weizhi } = exp,
            clear = ( id, name ) => {
            ...
            success = ( id, name, data ) => {
                ...
                id == "wallabag" && ( storage.secret[id]["access_token"] = {
                  access_token : data.access_token,
                  refresh_token : data.refresh_token,
                  expireDateMs : data.expireDateMs
                });
                storage.Safe( () => {
                    ...
                    id == "wallabag"    && this.setState({ secret: storage.secret, wallabag: false });
        ...
        if ( state == "wallabag" && !flag && !storage.secret.wallabag.username ) {
          this.setState({ wallabag: !this.state.wallabag });
          return;
        }
        ...
        if ( !value ) {
          ...
          if ( state == "wallabag" ) {
            this.props.wallabag.username = "";
            this.props.wallabag.password = "";
            this.props.wallabag.client_id = "";
            this.props.wallabag.client_secret = "";
            this.props.wallabag.server = "";
            //this.props.wallabag.access_token = "";
            //this.props.wallabag.refresh_token = "";
          }
        ...
        switch ( state ) {
          ...
          case "wallabag":
            wallabag.Auth(
              ...
              this.props.wallabag.client_id,
              this.props.wallabag.client_secret,
              this.props.wallabag.server, (result, error) => {
                if ( error ) {
                  failed( "授权错误，请重新授权。", wallabag.id, wallabag.name );
                } else {
                  let nowDate = new Date(Date.now());
                  success( "wallabag", "Wallabag", {
                    username: this.props.wallabag.username,
                    password: this.props.wallabag.password ,
                    client_id: this.props.wallabag.client_id,
                    client_secret: this.props.wallabag.client_secret,
                    server: this.props.wallabag.server,
    ...
    wallabagOnChange( state, value ) {
        this.props.wallabag[state] = value;
    }
    ...
    render() {
        ...
        if ( this.state.secret ) {
            ...
            auth = <div>
              ...
              <div className="version-tips" data-version="1.1.3" data-hits="wallabag">
                <Switch width="100%" checked={ this.state.secret.wallabag && this.state.secret.wallabag.username != "" && this.state.secret.wallabag.password ? true : false }
                        thumbedColor="#3F51B5" trackedColor="#7986CB" waves="md-waves-effect"
                        label={ this.state.secret.wallabag && this.state.secret.wallabag.username != "" ? "已授权 Wallabag，是否取消授权？" : "是否连接并授权 Wallabag ？" }
                        onChange={ (s)=>this.onChange( "wallabag", s ) } />
              </div>
              { this.state.wallabag &&
              <div ref="wallabag">
                <div style={{ "display": "flex", "flex-direction": "row" }}>
                  <TextField
                    placeholder="用户名"
                    onChange={ (evt)=>this.wallabagOnChange( "username", evt.target.value ) }
                  />
                  <TextField
                    password={true}
                    placeholder="密码"
                    onChange={ (evt)=>this.wallabagOnChange( "password", evt.target.value ) }
                  />
                  <TextField
                    placeholder="客户端ID"
                    onChange={ (evt)=>this.wallabagOnChange( "client_id", evt.target.value ) }
                  />
                  <TextField
                    password={true}
                    placeholder="客户端密钥"
                    onChange={ (evt)=>this.wallabagOnChange( "client_secret", evt.target.value ) }
                  />
                  <TextField
                    placeholder="服务器地址"
                    onChange={ (evt)=>this.wallabagOnChange( "server", evt.target.value ) }
                  />

                </div>

                <Button type="raised" width="100%" style={{ "margin": "0" }}
                        text="绑定 Wallabag 的信息"
                        color="#fff" backgroundColor="#3F51B5"
                        waves="md-waves-effect md-waves-button"
                        onClick={ (s)=>this.onChange( "wallabag", s, "login" ) } />
